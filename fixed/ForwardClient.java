...
public class ForwardClient {
	...
	private static void tellUser(ServerSocket listensocket) throws UnknownHostException {
		...
		System.out.println("Waiting for incoming connections at " + InetAddress.getLocalHost().getHostName() + ":"
				+ listensocket.getLocalPort());
	}

	/*
	 * Set up client forwarder. Run handshake negotiation, then set up a listening
	 * socket and start port forwarder thread.
	 */
	static public void startForwardClient() throws IOException {
		...
		if (!doHandshake(handshakeSocket)) {
			return;
		}

		/*
		 * Create a new listener socket for the proxy port. This is where the user will
		 * connect.
		 */
		ServerSocket proxySocket = new ServerSocket(Integer.parseInt(arguments.get("proxyport")));

		/*
		 * Tell the user, so the user knows the we are listening at the proxy port.
		 */
		tellUser(proxySocket);

		/*
		 * Set up port forwarding between proxy port and session host/port that was
		 * learned from the handshake.
		 */
		ForwardServerClientThread forwardThread = new ForwardServerClientThread(proxySocket, clientHandshake.sessionHost,
				clientHandshake.sessionPort, clientHandshake.sessionEncrypter, clientHandshake.sessionDecrypter, false);
		/*
		 * Launch the fowarder
		 */
		forwardThread.start();
	...
	public static void main(String[] args) {
		try {
			...
			if (arguments.get("handshakehost") == null || arguments.get("handshakeport") == null) {
				throw new IllegalArgumentException("Handshake host or port not specified");
			}
